	...
	switch (commandName) {
		...
		case "respond":
			...
			case "reply":
				...
				break;
				case "insultuser":
					log(LogLevel.Debug, `Attempting to run (Context-intergration) [insultuser]`)
					try {
						await interaction.deferReply()
						try {
							if (fs.existsSync(`./cache/system-message/system-message-${interaction.channel.id}.txt`)) {
					...
					var prompt = `${init_prompt}${currentutctime}${currentsystime}\nYou are here to insult the user ${interaction.targetUser}` 
					log(LogLevel.Debug, prompt)
						...
						var responseEmbed = {
							color: 0xE42831,
							title: 'Petty Insult user command results',
							author: {
								name: 'Alice',
								url: 'https://ethmangameon.github.io/alice-app/home.html',
							},
					...
					log(LogLevel.Debug, `Finished responding to (Context-intergration) [insultuser]`)
					break;
		case "upscale":
